 // ====== CONFIGURAÇÃO ====== contantes que sempre serão as mesmas quando usadas
  ...
  // ====== ELEMENTOS ======
  //const cells = Array.from(document.querySelectorAll(".holes .hole-cell")); modificado para incluir as células do grid
  // antes: const cells = Array.from(document.querySelectorAll(".holes .hole-cell")); modificado para incluir as células do grid
const cells = Array.from(document.querySelectorAll(".holes .hole-cell, .holes-grid .cell"));
  ...
  cells.forEach((td, idx) => {
    ...
    //td.addEventListener("click", () => {
      ...
      if (active.has(idx)) {
        // ACERTOU
        clearTimeout(active.get(idx)); // cancela o "sumir sozinha"
        active.delete(idx);
        hideMole(td);
        score.acertos++;
        updateSaldo();
  ...
  if (!stopBtn.disabled) {
    ...
    // (se já existe stopGame() no seu IIFE, apenas chame)
    // Caso esteja dentro do escopo, chame direto:
    // stopGame();

    // Se stopGame estiver fechado no escopo, coloque o listener dentro do mesmo
    // arquivo, junto dos outros handlers (onde você tem acesso a stopGame()).
    stopGame();
  }
});
  function startGame() {
